...
class StartController @Inject() (
  ...
  val start: Action[AnyContent] = authWithRetrievalsAction.async { implicit request =>
    val result = for {
      ...
      isConfirmDetailsInSession = maybeStoredSession.fold(false)(_.mapAsIndividual(_.isConfirmDetailsInSession))
      newSession                = loginData match {
                                    case i: IndividualLoginData if isConfirmDetailsInSession =>
                                      IndividualHECSession
                                        .newSession(i)
                                        .copy(unexpiredTaxChecks = existingTaxChecks, isConfirmDetailsInSession = true)
                                    case i: IndividualLoginData                              =>
                                      IndividualHECSession
                                        .newSession(i)
                                        .copy(unexpiredTaxChecks = existingTaxChecks)
